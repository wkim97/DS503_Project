    ...
    for epoch in range(DALLE_CFG.EPOCHS):
        for i, (text, images, mask) in enumerate(dl):
            ...
            if i % 100 == 0:
                ...
                print('Epoch: {}/{}\t Iter: {}/{}\t Loss: {:.4f}\t Time: {:.2f} sec'.format(
                    epoch, DALLE_CFG.EPOCHS, i, int(len(ds) / args.batch_size), loss.item(), time_taken))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_folder",
        type=str,
        default="/mnt/hdd1/wkim/DS503/KoDALLE/data/montage/preprocessed/images",
        help="",
    )
    parser.add_argument(
        "--text_folder", type=str, default="/mnt/hdd1/wkim/DS503/KoDALLE/data/montage/preprocessed/labels",
    )
