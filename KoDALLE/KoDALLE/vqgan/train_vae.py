    ...
    for epoch in range(args.epochs):
        for i, images in enumerate(dl):
            ...
            if i % 100 == 0:
                ...
                start_time = time.time()
                print('Epoch: {}/{}\t Iter: {}/{}\t Time: {:.2f} sec\t Loss: {:.4f}'.format(
                    epoch, args.epochs, i, int(len(ds)/args.batch_size), time_taken, loss.item()))
                ...
                temp = max(
                    temp * math.exp(-float(VAE_CFG.ANNEAL_RATE) * global_step),
                    VAE_CFG.TEMP_MIN,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vae_config",
        type=str,
        default="../configs/VQGAN_config.yml",
        help="",
    )
    ...
    parser.add_argument(
        "--image_folder",
        type=str,
        default='../data/montage/preprocessed',
    ...
    with open(args.vae_config, "r") as f:
        vae_config = yaml.load(f, yaml.Loader)
    ...
    vae_params = dict(
        ...
        codebook_dim=VAE_CFG.EMB_DIM,
        hidden_dim=VAE_CFG.HID_DIM,
        num_resnet_blocks=VAE_CFG.NUM_RESNET_BLOCKS,
    )

    vae = DiscreteVAE(
        **vae_params,
        smooth_l1_loss=VAE_CFG.SMOOTH_L1_LOSS,
        kl_div_loss_weight=VAE_CFG.KL_LOSS_WEIGHT,
    )
    vae = vae.to(device)
    ...
    model_config = dict(
        ...
        smooth_l1_loss=VAE_CFG.SMOOTH_L1_LOSS,
        num_resnet_blocks=VAE_CFG.NUM_RESNET_BLOCKS,
        kl_loss_weight=VAE_CFG.KL_LOSS_WEIGHT,
    )
